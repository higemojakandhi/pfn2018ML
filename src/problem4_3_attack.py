...
for i in range(num_data):
    ...
    g = [networks['param_' + str(i)].gradient(X[i], T[i]) for i in range(num_file)]

    eps = [Array.fromList([math.copysign(ep0,item[0]) for item in zip(g[i].rows)]) for i in range(num_file)]
    sum = Array.zeros_like(eps[0])
    for ep in eps:
        sum += ep
